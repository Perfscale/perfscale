...
#[test]
#[file_serial(heavy_io)]
fn bench_writes_report_file_with_output_flag() {
    ...
    assert!(written.contains("## Results"), "file content:\n{written}");
}

#[test]
fn bench_unknown_engine_fails_with_hint() {
    assert_cmd::Command::new(cargo_bin("perfscale"))
        .args(["bench", "--engines", "wrk"])
        .timeout(Duration::from_secs(30))
        .assert()
        .failure()
