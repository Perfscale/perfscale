...
- **`--summary-export` for gRPC and WebSocket runs**: the summary parser now falls back to the native protocol series (`grpc_req_duration`, `ws_msg_rtt`, `*_msgs_sent`, `*_req_failed`) when no `http_req_*` metrics were recorded, so pure gRPC and non-HTTP WebSocket runs export real metrics instead of `summary: null`. Protocol summary lines are also forwarded to `--report` endpoints.
